...
func newHomeCmd() *cobra.Command {
	...
	switchCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runWithClient(cmd, "switch failed", func(ctx context.Context, c *fritz.Client) error {
				var on bool
	...
	tempCmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runWithClient(cmd, "set temp failed", func(ctx context.Context, c *fritz.Client) error {
				var temp float64
				switch strings.ToLower(args[1]) {
				case "on":
					temp = 254
