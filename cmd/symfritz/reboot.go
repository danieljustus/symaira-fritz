...
func newRebootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return runWithClient(cmd, "reboot failed", func(ctx context.Context, c *fritz.Client) error {
				svc := fritz.Service{
					Type:       "urn:dslforum-org:service:DeviceConfig:1",
					ControlURL: "/upnp/control/deviceconfig",
				}
				if _, err := c.Call(ctx, svc, "Reboot", nil); err != nil {
					return wrapFritzError(err, "reboot failed")
				}
