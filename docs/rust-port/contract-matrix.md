...
| DIST-004 | Value gate | release-built Rust candidate + pinned Go oracle `b1491793` + strict fake box | Go oracle built from `b1491793` | >=20% size or RSS gain and <=10% startup/fake-box p95 regression | `scripts/value_gate.py` + `value-gate-a05811a8.json` (provenance-bound); `value-gate-20260905.json` is unbindable historical evidence | macOS arm64 measured; Linux/Windows not measured | measured values | PASS (macOS arm64 only, candidate `a05811a8`) |
