...
| MCP-003 | Tool calls | success/validation/backend failures | Go fixture oracle | JSON-RPC IDs, content text strings, `isError` behavior | same + unit tests | all | parsed semantic | PASS |
| MCP-004 | Stdio hygiene | initialize/list/call/notifications/malformed frames | Go corekit oracle | only protocol frames on stdout; logs on stderr | raw-frame process harness | all | raw framing + semantic | PASS |
