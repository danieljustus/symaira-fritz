...
class ReleaseWorkflowPolicyTests(unittest.TestCase):
    ...
    def test_artifact_actions_are_immutable_and_never_empty(self) -> None:
        self.assertIn("actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a # v7.0.1", WORKFLOW_TEXT)
        self.assertIn("actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c # v8.0.1", WORKFLOW_TEXT)
    ...
    def test_release_identity_is_semver_and_sha_bound(self) -> None:
        ...
        self.assertEqual(WORKFLOW_TEXT.count('[[ "$REF" != refs/tags/v* ]]'), 2)
        self.assertEqual(WORKFLOW_TEXT.count("create_args+=(--verify-tag)"), 1)
        self.assertNotIn('create_args+=(--target "$GITHUB_SHA")', WORKFLOW_TEXT)
    ...
    def test_release_readback_covers_assets_hashes_and_sboms(self) -> None:
        ...
        self.assertIn('for local_asset in dist/*', WORKFLOW_TEXT)
        self.assertIn('cmp -- "$local_asset" "$public_asset"', WORKFLOW_TEXT)
