# Symaira Fritz public asset kit

Editable, local-first communications assets for the CLI/MCP product. This supplements the approved A3 brand handoff in `../a3/`; it does not replace that signet or imply a native app, cloud dashboard, or Apple app icon.
...
- Sources: the local OpenDesign `symaira-fritz` Brand & Assets handoff (version 0.23.0). Each copied source/export is bound by SHA-256 and dimensions in `manifest.json`. The monochrome variant is derived from the foreground mark by using its light foreground color. The terminal illustration's command and output were corrected to a **clearly synthetic text-format** example, then re-rendered from its SVG.
...
- SVGs are transparent for the foreground marks and contain no external image URLs or scripts. Social/release/README exports use synthetic copy and the public GitHub repository URL; the terminal illustration says explicitly that it contains no router data.
